...
- **API Client**: `lib/seoApi.ts` — typed client functions for all SEO endpoints, including new interfaces: `SeoOverviewKpis`, `PageQueryPerformance`, `PageOptimizationRec`, `InternalLinkRec`, `ContentPipeline`, `AhrefsKeywordGap`, `AhrefsBacklinkGap`, `AhrefsTopPage`, `ClusterPriority`
- **Intelligence API**: `app/api/proxy/seo/intelligence/route.ts` — consolidated route with 10 views: `overview-kpis`, `page-performance`, `page-optimization`, `internal-links`, `content-pipeline`, `competitive-gap`, `competitive-backlinks`, `competitive-pages`, `cluster-priorities`, `competitors-list`. Includes DB config guard, hardened limit parsing, DISTINCT ON dedup for Ahrefs tables.
...
- **Data Sources**: `seo_page_query_performance_live`, `seo_page_optimization_recommendations`, `internal_link_recommendations`, `seo_content_artifacts`, `seo_generation_candidates`, `seo_generation_events`, `raw_ahrefs_keyword_gap`, `raw_ahrefs_backlink_gap`, `raw_ahrefs_top_pages`