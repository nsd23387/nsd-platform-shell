...
- **nsd-integrations Competitive API**: Producer hosted at `https://nsd-integrations.vercel.app`, exposing `GET /api/v1/competitive/{summary,weekly-insights/latest,pages/changes}`. Consumed via the server-side proxy at `/api/competitive/[endpoint]`. Requires `API_SECRET_KEY` (same env-var name and same value as `API_SECRET_KEY` in the producer's Vercel project — Vercel project env namespaces are isolated, so the value must be pasted into both projects) set in both Replit Secrets (dev) and the **`nsd-command-center`** Vercel project's Environment Variables (Production + Preview + Development) — the consumer Vercel project that serves `https://analytics.neonsignsdepot.com`. Supabase `competitive.*` schema is the underlying source of truth; the producer is the contract boundary — do not read `competitive.*` directly from this shell without governance review.
