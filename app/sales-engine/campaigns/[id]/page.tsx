...
import { getCampaign, requestCampaignRun, duplicateCampaign } from '../../lib/api';
...
export default function CampaignDetailPage() {
  ...
  const handleRunCampaign = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 'run_requested' || response.status === 'queued') {
        setRunRequestMessage('Execution requested');
  ...
  }, [campaignId, isRunRequesting, refreshExecution]);
...
function ExecutionStatusBanner({
  ...
  let icon: 'clock' | 'refresh' | 'check' | 'warning' = 'clock';
