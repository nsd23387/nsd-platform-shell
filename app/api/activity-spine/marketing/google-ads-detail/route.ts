...
async function getCampaignDaily(start: string, end: string) {
  const { rows } = await pool.query(`
    SELECT
      ...
      CASE WHEN SUM(clicks) > 0 THEN SUM(cost)::numeric / SUM(clicks) ELSE 0 END AS average_cpc
    FROM analytics.metrics_google_ads_campaign_daily
    WHERE metric_date BETWEEN $1::date AND $2::date
    GROUP BY metric_date, campaign_id, campaign_name
...
async function getCampaignSummary(start: string, end: string) {
  const { rows } = await pool.query(`
    SELECT
      ...
      SUM(ads.conversion_value)::numeric AS conversion_value,
      COALESCE(SUM(q.paid_revenue_usd), 0)::numeric AS quote_revenue_usd,
      CASE
        WHEN SUM(ads.cost) > 0 THEN COALESCE(SUM(q.paid_revenue_usd), 0)::numeric / SUM(ads.cost)
        ELSE 0
      END AS estimated_roas_qms
    FROM analytics.metrics_google_ads_campaign_daily ads
    LEFT JOIN marketing.google_ads_quote_performance q
      ON q.report_date = ads.metric_date
     AND q.google_campaign_id = ads.campaign_id
    WHERE ads.metric_date BETWEEN $1::date AND $2::date
    GROUP BY ads.campaign_id, ads.campaign_name
  ...
  return {
    campaigns: rows.map(r => ({
      ...
      roas: Number(r.estimated_roas_qms) || (Number(r.cost) > 0 ? Number(r.conversion_value) / Number(r.cost) : 0),
    })),
    totals: {
      spend: totalSpend,
      clicks: totalClicks,
      ...
      roas: totalSpend > 0
        ? rows.reduce((sum, r) => sum + Number(r.quote_revenue_usd || 0), 0) / totalSpend
        : 0,
    },
  };
}
...
async function getKeywords(start: string, end: string, campaignId?: string | null) {
  ...
  if (campaignId) {
    ...
    campaignFilter = `AND campaign_id = $3`;
  }
  ...
  const { rows } = await pool.query(`
    ...
    FROM analytics.metrics_google_ads_keyword_daily
    WHERE metric_date BETWEEN $1::date AND $2::date
        ${campaignFilter}
    GROUP BY
      keyword_text,
...
async function getSearchTerms(start: string, end: string, campaignId?: string | null, sortBy?: string | null) {
  ...
  const orderCol = sortBy === 'clicks' ? "SUM(clicks)" : "SUM(cost)";

  const { rows } = await pool.query(`
    SELECT
      COALESCE(NULLIF(search_term, ''), '(not available)') AS search_term,
      campaign_id,
      campaign_name_raw AS campaign_name,
      ad_group_id,
      ad_group_name_raw AS ad_group_name,
      SUM(clicks)::bigint AS clicks,
      SUM(impressions)::bigint AS impressions,
      SUM(cost)::numeric AS cost,
      SUM(conversions)::numeric AS conversions,
      SUM(conversion_value)::numeric AS conversion_value
    FROM analytics.metrics_google_ads_search_term_daily
    WHERE metric_date BETWEEN $1::date AND $2::date
        ${campaignFilter}
    GROUP BY
      COALESCE(NULLIF(search_term, ''), '(not available)'),
      campaign_id,
      campaign_name_raw,
      ad_group_id,
      ad_group_name_raw
  ...
  try {
    const { rows } = await pool.query(`
      SELECT
        ...
        SUM(conversions)::numeric AS conversions,
        SUM(conversion_value)::numeric AS conversion_value
      FROM analytics.metrics_google_ads_campaign_conversions
      WHERE metric_date BETWEEN $1::date AND $2::date
