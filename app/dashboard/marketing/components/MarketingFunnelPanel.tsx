...
import { text, border, background, violet, indigo, chartColors } from '../../../../design/tokens/colors';
...
export function MarketingFunnelPanel({ funnel, loading, error }: Props) {
  ...
  const stages: Stage[] = [
    ...
    { label: 'Pipeline Value', value: totalPipeline, format: formatCurrency, color: chartColors[2], gradient: `linear-gradient(135deg, #38bdf8, #0284c7)` },
  ];
  ...
  return (
    <DashboardSection title="Conversion Funnel" description="Page views through submissions to pipeline value.">
      <div style={{ backgroundColor: background.surface, border: `1px solid ${border.default}`, borderRadius: radius.xl, padding: space['6'] }} data-testid="panel-funnel">
            ...
            return (
              <div key={stage.label} data-testid={`funnel-stage-${i}`}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: space['1.5'] }}>
                  ...
                  <span style={{ fontFamily: fontFamily.body, fontSize: fontSize['2xl'], fontWeight: fontWeight.semibold, color: text.primary }}>
                    {stage.format(stage.value)}
                  </span>
                </div>
                <div style={{ backgroundColor: background.muted, borderRadius: radius.full, height: 24, overflow: 'hidden', position: 'relative' }}>
                  <div style={{
