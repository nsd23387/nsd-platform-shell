...
import { getSeoOverviewKpis, getPagePerformance } from '../../../lib/seoApi';
import type { SeoOverviewKpis, PageQueryPerformance } from '../../../lib/seoApi';
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          setKpis(k); setTopPages(tp); setLoading(false);
        }
      } catch (err: unknown) {
