    ...
    <div style={{
      ...
      {expanded && (
        <div style={{ padding: `0 ${space['4']} ${space['4']}`, borderTop: `1px solid ${tc.border.default}` }}>
          ...
          {/* Reviewer rationale (shown once a human has decided) */}
          {action.human_notes && (
            <div data-testid={`text-reviewer-rationale-${action.id}`} style={{ padding: space['3'], backgroundColor: tc.background.muted, borderRadius: radius.md, marginBottom: space['3'] }}>
              <div style={{ fontFamily: fontFamily.body, fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.05em', color: tc.text.muted, fontWeight: fontWeight.medium, marginBottom: space['1'] }}>
                Reviewer rationale{action.human_decision ? ` (${action.human_decision})` : ''}
              </div>
              <div style={{ fontFamily: fontFamily.body, fontSize: fontSize.sm, color: tc.text.secondary, lineHeight: lineHeight.relaxed }}>
                {action.human_notes}
              </div>
          ...
          {/* Reject rationale form — rationale is required and is saved back to the engine */}
          {canAct && rejecting && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: space['2'] }}>
              <label htmlFor={`reject-rationale-${action.id}`} style={{ fontFamily: fontFamily.body, fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.05em', color: tc.text.muted, fontWeight: fontWeight.medium }}>
                Why are you rejecting this? (required — feeds the SEO engine)
              </label>
              <textarea
                id={`reject-rationale-${action.id}`}
                value={rationale}
                onChange={(e) => setRationale(e.target.value)}
                placeholder="e.g. Off-brand keyword — 'LED' attracts the wrong customers; target 'neon' instead."
                rows={3}
                disabled={isLoading}
                data-testid={`input-reject-rationale-${action.id}`}
                style={{ width: '100%', padding: space['2.5'], fontFamily: fontFamily.body, fontSize: fontSize.sm, color: tc.text.primary, backgroundColor: tc.background.surface, border: `1px solid ${tc.border.default}`, borderRadius: radius.md, resize: 'vertical', boxSizing: 'border-box' }}
              />
              <div style={{ display: 'flex', gap: space['2'] }}>
                <button
                  disabled={isLoading || rationale.trim().length === 0}
                  onClick={() => onReject(action.id, rationale.trim())}
                  data-testid={`button-confirm-reject-${action.id}`}
                  style={{ flex: 1, padding: `${space['2.5']} ${space['4']}`, fontFamily: fontFamily.body, fontSize: fontSize.sm, fontWeight: fontWeight.semibold, color: '#fff', backgroundColor: '#DC2626', border: 'none', borderRadius: radius.md, cursor: (isLoading || rationale.trim().length === 0) ? 'not-allowed' : 'pointer', opacity: (isLoading || rationale.trim().length === 0) ? 0.5 : 1 }}
                >
                  Confirm reject
                </button>
                <button
                  disabled={isLoading}
                  onClick={() => { setRejecting(false); setRationale(''); }}
                  data-testid={`button-cancel-reject-${action.id}`}
                  style={{ padding: `${space['2.5']} ${space['4']}`, fontFamily: fontFamily.body, fontSize: fontSize.sm, fontWeight: fontWeight.medium, color: tc.text.secondary, backgroundColor: tc.background.muted, border: `1px solid ${tc.border.default}`, borderRadius: radius.md, cursor: isLoading ? 'wait' : 'pointer', opacity: isLoading ? 0.5 : 1 }}
                >
                  Cancel
                </button>
              </div>
...
function ActionsContent() {
  ...
  const handleReject = async (id: string, rationale: string) => {
    setActionLoading(id);
    try {
      await rejectSeoAction(id, rationale);
      loadActions();
    } catch (err) {
