...
import type { SeoPageEnhancementMember, SeoPageEnhancementPackage } from '../../../../lib/seoApi';
...
function fieldTone(field: string): 'good' | 'bad' | 'warn' | 'info' | 'violet' | 'neutral' {
  switch (field) {
...
function RecommendationsContent() {
  ...
  }, [activePackage, approvePackage, filtered.length, rejectPackage]);

  return (
    <div style={{ padding: space['6'], maxWidth: 1180, margin: '0 auto' }}>
      <div style={{ marginBottom: space['5'] }}>
        <h1 style={{ fontFamily: fontFamily.display, fontSize: '24px', fontWeight: fontWeight.semibold, color: tc.text.primary, margin: 0 }}>Pages to Review</h1>
        <p style={{ fontFamily: fontFamily.body, fontSize: '13px', color: tc.text.muted, marginTop: '4px' }} data-testid="text-recommendations-subtitle">
          Review one page package at a time. Each card bundles the page&apos;s title, meta, H1, alt, and schema changes into one decision and one 30/60-day evaluation clock.
        </p>
      </div>

      <div style={{ display: 'flex', gap: space['2'], flexWrap: 'wrap', alignItems: 'center', marginBottom: space['4'] }}>
        <input
          value={search}
      ...
      <div style={{ display: 'flex', gap: space['2'], flexWrap: 'wrap', marginBottom: space['4'], fontFamily: fontFamily.body, fontSize: '12px', color: tc.text.muted }}>
